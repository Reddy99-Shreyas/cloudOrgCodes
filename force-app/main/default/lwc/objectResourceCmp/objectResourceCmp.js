...
export default class ObjectResourceCmp extends LightningElement {
    ...
    handleSaveClick(){
        ...
        .then((result)=>{
            ...
            this.showToast('Resource Saved','Resources Saved Successfully','success');
        })
        .catch((error)=>{
            console.log('Error: '+JSON.stringify(error));
    ...
    handleResourceName(event){
        this.getResource.Name = event.target.value;
    }
    ...
    @wire(getPicklistValues,{recordTypeId: '$resourceObj.data.defaultRecordTypeId',fieldApiName: CLIENT_INTERVIEW_REQUIRED,})
    resClientInterReq;

    @wire(getPicklistValues,{recordTypeId: '$resourceObj.data.defaultRecordTypeId',fieldApiName: DEPARTMENT,})
    resDepartment;

    @wire(getPicklistValues,{recordTypeId: '$resourceObj.data.defaultRecordTypeId',fieldApiName: DEPARTMENT,})
    resDepartment;

    @wire(getPicklistValues,{recordTypeId: '$resourceObj.data.defaultRecordTypeId',fieldApiName: LOCATION,})
    resLocation;

    @wire(getPicklistValues,{recordTypeId: '$resourceObj.data.defaultRecordTypeId',fieldApiName: TYPE,})
    resType;

    @wire(getPicklistValues,{recordTypeId: '$resourceObj.data.defaultRecordTypeId',fieldApiName: EMPLOYMENT_TYPE,})
    resEmpType;

    @wire(getPicklistValues,{recordTypeId: '$resourceObj.data.defaultRecordTypeId',fieldApiName: JOB_LOCATION,})
    resJobLoc;

    @wire(getPicklistValues,{recordTypeId: '$resourceObj.data.defaultRecordTypeId',fieldApiName: CLIENT_NAME,})
    resClientName;

    @wire(getPicklistValues,{recordTypeId: '$resourceObj.data.defaultRecordTypeId',fieldApiName: EXPERTISE_AREA,})
    resExpArea;

    @wire(getPicklistValues,{recordTypeId: '$resourceObj.data.defaultRecordTypeId',fieldApiName: ONSITE_INTERVIEW_REQUIRED,})
    resOnsiteInterReq;

    @wire(getPicklistValues,{recordTypeId: '$resourceObj.data.defaultRecordTypeId',fieldApiName: LEVEL_GRADE,})
    resLevelGrade;

    @wire(getPicklistValues,{recordTypeId: '$resourceObj.data.defaultRecordTypeId',fieldApiName: PRIORITY,})
    resPriority;

    @wire(getPicklistValues,{recordTypeId: '$resourceObj.data.defaultRecordTypeId',fieldApiName: TITLE,})
    resTitle;

    @wire(getPicklistValues,{recordTypeId: '$resourceObj.data.defaultRecordTypeId',fieldApiName: BILLABILITY,})
    resBillability;

    @wire(getPicklistValues,{recordTypeId: '$resourceObj.data.defaultRecordTypeId',fieldApiName: SHIFT_TIMINGS,})
    resShiftTimings;
